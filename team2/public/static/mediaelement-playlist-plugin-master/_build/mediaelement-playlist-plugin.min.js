...
! function(a) {
    a.extend(mejs.MepDefaults, {
        ...
        buildloop: function(b, c, d, e) {
            var f = this,
                g = a('<div class="mejs-button mejs-loop-button ' + (b.options.loop ? "mejs-loop-on" : "mejs-loop-off") + '"><button type="button" aria-controls="' + b.id + '" title="' + b.options.loopText + '"></button></div>').appendTo(c).click(function() {
                    b.options.loop = !b.options.loop, a(e).trigger("mep-looptoggle", [b.options.loop]), b.options.loop ? g.removeClass("mejs-loop-off").addClass("mejs-loop-on") : g.removeClass("mejs-loop-on").addClass("mejs-loop-off")
                });
        ...
        buildshuffle: function(b, c, d, e) {
            var f = this,
                g = a('<div class="mejs-button mejs-shuffle-button ' + (b.options.shuffle ? "mejs-shuffle-on" : "mejs-shuffle-off") + '"><button type="button" aria-controls="' + b.id + '" title="' + b.options.shuffleText + '"></button></div>').appendTo(c).click(function() {
        ...
        buildprevtrack: function(b, c, d, e) {
            var f = this,
                g = a('<div class="mejs-button mejs-prevtrack-button mejs-prevtrack"><button type="button" aria-controls="' + b.id + '" title="' + b.options.prevText + '"></button></div>');
        ...
        buildnexttrack: function(b, c, d, e) {
            var f = this,
                g = a('<div class="mejs-button mejs-nexttrack-button mejs-nexttrack"><button type="button" aria-controls="' + b.id + '" title="' + b.options.nextText + '"></button></div>');
            g.appendTo(c).click(function() {
        ...
        buildplaylist: function(b, c, d, e) {
            var f = this,
                g = a('<div class="mejs-button mejs-playlist-button ' + (b.options.playlist ? "mejs-hide-playlist" : "mejs-show-playlist") + '"><button type="button" aria-controls="' + b.id + '" title="' + b.options.playlistText + '"></button></div>');
            g.appendTo(c).click(function() {
        ...
        buildplaylistfeature: function(b, c, d, e) {
            ...
            a("#" + b.id).find(".mejs-mediaelement source").each(function() {
                if (h = a(this).parent()[0].canPlayType(this.type), k || "maybe" !== h && "probably" !== h || (k = this.type), k && this.type === k && "" !== a.trim(this.src)) {
                    var b = {};
                    b.source = a.trim(this.src), b.name = "" !== a.trim(this.title) ? a.trim(this.title) : i(b.source), b.poster = a(this).data("poster"), j.push(b)
                }
            });
            for (var l in j) {
            ...
            b.videoSliderIndex = 0, d.find(".mejs-playlist").append(n), d.find(".mejs-playlist").append(o), a("#" + b.id + ".mejs-container.mep-slider").find(".mejs-playlist ul li").css({
                ...
                b.container.removeClass("mep-paused").addClass("mep-playing"), a(e).is("video") //&& f.togglePlaylistDisplay(b, d, e, "hide")
            }, !1), e.addEventListener("play", function() {
                a(b.media).is("audio") && d.find(".mejs-poster").show()
        ...
        playNextTrack: function() {
            var a, b = this,
                ...
                d = c.filter(".current"),
                e = c.not(".played");
            if (e.length < 1 && (d.removeClass("played").siblings().removeClass("played"), e = c.not(".current")), b.options.shuffle) {
                var f = Math.floor(Math.random() * e.length);
                a = e.eq(f)
            } else a = d.next(), a.length < 1 && b.options.loop && (a = d.siblings().first());
            1 == a.length && (a.addClass("played"), b.playTrack(a))
        },
        playPrevTrack: function() {
            ...
            if (e.length < 1 && (d.removeClass("played"), e = c.not(".current")), b.options.shuffle) {
                var f = Math.floor(Math.random() * e.length);
                a = e.eq(f)
            } else a = d.prev(), a.length < 1 && b.options.loop && (a = d.siblings().last());
        ...
        togglePlaylistDisplay: function(b, c, d, e) {
            var f = this;
            b.options.playlist = e ? "show" === e ? !0 : !1 : !b.options.playlist, a(d).trigger("mep-playlisttoggle", [b.options.playlist]), b.options.playlist ? (c.children(".mejs-playlist").fadeIn(), f.playlistToggle.removeClass("mejs-show-playlist").addClass("mejs-hide-playlist")) : (c.children(".mejs-playlist").fadeOut(), f.playlistToggle.removeClass("mejs-hide-playlist").addClass("mejs-show-playlist"));
            toggleWideVideo();
        }
    })
}(mejs.$);

function toggleWideVideo(){
    var toggleWidth = $("#mep_0").width() == 530 ? "100%" : "530px";
    var toggleHeight = $("#mep_0").height() == 300 ? "400px" : "300px";
    if(toggleWidth == "100%"){
            $('#video-wrapper').css('width', '100%');
            $('#video-wrapper').css('height', '400px');

    } else {
            $('#video-wrapper').css('width', '530px');
            $('#video-wrapper').css('height', '300px');
    }
}
